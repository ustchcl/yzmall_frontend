...
-- | app capabilities.
module Yzmall.Api.Request where
...
import Yzmall.Api.Endpoint (Endpoint, endpointCodec)
import Yzmall.Data.Phone (Phone(..))
import Yzmall.Utils (getOrElse)

foreign import json2FormData :: Json -> FormData
foreign import json2String :: Json -> String
...
defaultRequest (BaseURL baseUrl) { endpoint, method } =
  ...
  , headers: 
    [ RequestHeader "Content-Type" "application/x-www-form-urlencode"
...
defaultRequestForm :: BaseURL -> RequestOptions -> Request Json
defaultRequestForm (BaseURL baseUrl) { endpoint, method } =
  { method: Left method 
  , url: baseUrl <> print endpointCodec endpoint
  , headers: [
     RequestHeader "Content-Type" "application/x-www-form-urlencoded"
  ]
  , content: RB.string <$> (json2String <$> body)
  , username: Nothing
  , password: Nothing
  , withCredentials: true
  , responseFormat: RF.json
...
type RegisterFields = { | AuthFieldsRep Unlifted (nickname :: String, invitor :: Maybe String, vcode :: String )}
type CreateVcode = { | AuthFieldsRep Maybe () }
type ResetPassWordFields = { | AuthFieldsRep Unlifted (vcode :: String) }
